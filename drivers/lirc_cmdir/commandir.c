...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 0)
static void * cmdir_probe (struct usb_device *dev,unsigned int ifnum,const struct usb_device_id *id);
static int cmdir_ioctl (struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg);
...
static int cmdir_open (struct inode *inode, struct file *file)
{
	...
	file->private_data = dev;	/* save our object in the file's private structure */
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 0)
static void * cmdir_probe(struct usb_device *udev,unsigned int ifnum,const struct usb_device_id *id)
{
...
static void cmdir_disconnect(struct usb_device *udev, void *ptr)
{
	...
	if (minor == rx_device)
	{
		...
		if (minor > 0) info("Active Receiver is on CommandIR #%d", rx_device);
	}
...
static int cmdir_check (int device_num)
{
	...
	interface = usb_find_interface(&cmdir_driver, USB_CMDIR_MINOR_BASE+device_num);
	if (!interface) 
	{
...
static void init_cmdir_var (int device_num)
{
	...
	for (i=0;i<device_num;i++) multiplier=multiplier*0x10;
...
static void reset_cmdir (int device_num)
{
	...
	for (i=1; i<MCU_CTRL_SIZE; i++) ctrl_buffer[i]='j';
...
ssize_t cmdir_read (unsigned char *buffer, size_t count)
{
...
#else
	struct usb_interface *interface;
	interface = usb_find_interface(&cmdir_driver, USB_CMDIR_MINOR_BASE+rx_device);
	if (!interface) 
	{
		/* also check without adding base, for devfs */
		interface = usb_find_interface(&cmdir_driver, rx_device);
	...
	if (!retval) 
	{
		...
		{
			curTXFill = buffer[2];	// current status of the TX buffer
			retval = count;
		}
	} 
	/* suppress errors */
	...
	*/
	...
	up (&dev->sem);
#endif
	// printk("CommandIR Reporting TX buffer at %d bytes. \n", curTXFill);
	return retval;
}	
...
static ssize_t cmdir_file_write (struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	...
	/* set as default - if non-specific error, won't keep calling this function */
	retval = count;
	...
	changeType = cNothing;
	equalsign = 0;
	for (i=0; i<MCU_CTRL_SIZE; i++) ctrl_buffer[i]='j';
	for (i=0; i<count; i++)
	{
		switch(local_buffer[i])
		{
		case 'X': case 'x':
			if ((i>0)&&((local_buffer[i-1]=='R')||(local_buffer[i-1]=='r')))
			{
				changeType = cRX;
			}
			break;
		case 'S': case 's': 
			if ((i>1)&&((local_buffer[i-1]=='E')||(local_buffer[i-1]=='e')))
			{
				if ((local_buffer[i-2]=='R')||(local_buffer[i-2]=='r'))
				{
					changeType = cRESET;
				}
			}
			break;
		case 'L': case 'l': 
			if ((i>0)&&((local_buffer[i-1]=='F')||(local_buffer[i-1]=='f')))
			{
				changeType=cFLASH;
			}
			break;
		case 'C': case 'c': 
			if((i>0)&&((local_buffer[i-1]=='L')||(local_buffer[i-1]=='l')))
			{
				changeType=cLCD;
			}
			break;
		case '=':
			if (changeType != cNothing) equalsign = i;
			break;
		case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7':
			if(equalsign>0)
			{
				...
				case cRESET:
					ctrl_buffer[0]=RESET_HEADER;
					retval=write_core(ctrl_buffer,MCU_CTRL_SIZE,
					       cmdir_write_bulk_callback,minor);
					if (retval != MCU_CTRL_SIZE)
					{
						if (retval==-ENODEV) 
							err("Device %d unplugged",minor);
						else
							err("Error on write to %d",minor);
						goto exit;
					}
					else
					{
				...
				case cFLASH:
					...
					info("Flashing indicators on device %d",minor);
					retval=write_core(ctrl_buffer,MCU_CTRL_SIZE,
					       cmdir_write_bulk_callback,minor);
					if (retval != MCU_CTRL_SIZE)
					{
				...
				case cRX:
					rx_device=minor;
					info("Default receiver set to %d",minor);
					break;
				case cLCD:
					lcd_device=minor;
					info("commandir: Default LCD set to %d",minor);
					break;
				default:
					break;
				}
...
int cmdir_write(unsigned char *buffer, int count, void *callback_fct, int usecdelay)
{
...
int add_cmdir_queue(unsigned char *buffer, int count, void *callback_vct, int usecdelay)
{
	...
	if((nexttofill + 1) % (QUEUELENGTH -1) == nexttosend){
		...
		// our buffer is full
		printk("Too many packets backlogged in CommandIR Queue.\n");
		return -1;
	}
	...
	//printk("Adding %d to queue at position %d.\n", count, nexttofill);
...
int send_queue()
{
	...
	if(send_status == SEND_IDLE){
		if(!(nexttofill == nexttosend)){
			...
			if(curTXFill < 190){ // always send if there's room, otherwise wait until room
				if(debug_commandir == 1){
					do_gettimeofday(&tp);
					printk("Sending packet data at %d\n", (int)tp.tv_usec);
				}
				ret = cmdir_write_queue(ourbuffers[nexttosend], ourbufferlengths[nexttosend], NULL);
				if(ret <= 0){
					// send failed - the device is either unplugged or full
					// nexttosend = (nexttosend + 1) % (QUEUELENGTH-1);
				...
				if(debug_commandir == 1){
					do_gettimeofday(&tp);
					printk("Not time to send yet - starting timer at %d.\n", (int)tp.tv_usec);
		...
		if(debug_commandir == 1) printk("Already sending\n"); // will try again on the callback
...
int wait_to_tx(int usecs){
	...
	if(debug_commandir == 1) printk("waittotx(%d)\n", usecs);
	if(usecs == 0) return 0; 
...
int cmdir_write_queue(unsigned char *buffer, int count, void *callback_fct)
{
	...
	case TX_HEADER_NEW:
		...
		if(curTXFill >= 190) {
			printk("TX buffer too full to send more TX data\n");
			return 0;
		}
		...
		{
			next_mask=0;
			if (transmitters & (0x01*multiplier)) next_mask |= TX1_ENABLE;
			if (transmitters & (0x02*multiplier)) next_mask |= TX2_ENABLE;
			if (transmitters & (0x04*multiplier)) next_mask |= TX3_ENABLE;
			if (transmitters & (0x08*multiplier)) next_mask |= TX4_ENABLE;

			if (next_mask > 0)
			{
				buffer[1] = next_mask;
				retval = write_core(buffer,count,callback_fct,i);
				if (retval != count)
				{
					if (retval==-ENODEV)
						err("Device %d not plugged in",i);
					else
						err("Write error to device %d", i);
					return retval;
				}
			}
...
static void cmdir_write_bulk_callback (struct urb *urb)
{
	if(debug_commandir == 1){
		do_gettimeofday(&tp);
		printk("cmdir_write_bulk_callback at %d\n", (int)tp.tv_usec);
	}
	...
	send_status = SEND_IDLE;
	// printk("cmdir_write_bulk_callback - set idle\n");
...
static void cmdir_write_bulk_callback(struct urb *urb, struct pt_regs *regs)
{
	...
	send_status = SEND_IDLE;
	if(debug_commandir == 1) printk("callback()\n");
