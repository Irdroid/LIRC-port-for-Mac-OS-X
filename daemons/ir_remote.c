/*      $Id: ir_remote.c,v 1.1 1999/04/29 21:17:00 columbus Exp $      */
...
#               endif
	...
	else
	{
		...
		pre=get_next_rec_buffer(0);
		printf("code: %llx\n",pre);
		if(remote->flags&REVERSE)
		{
			pre=reverse(pre,remote->pre_data_bits+remote->bits
				    +remote->post_data_bits);
		}
